...
qemu-system-x86_64 -enable-kvm -nographic -serial mon:telnet::4444,server -smp 4 -m 8G -drive file=Fedora-Cloud-Base-32-1.6.x86_64.qcow2,if=virtio -s -nic user,hostfwd=tcp::52-:22 #-kernel bzImage -append 'root=/dev/vda console=ttyS0,115200 nokaslr debug'


qemu-system-x86_64 -enable-kvm -nographic -serial mon:telnet::4444,server -device vhost-vsock-pci,id=vhost-vsock-pci0,guest-cid=3 -smp 4 -m 8G -drive file=Fedora-Cloud-Base-32-1.6.x86_64.qcow2,if=virtio -s -nic user,hostfwd=tcp::52-:22 #-kernel bzImage -append 'root=/dev/vda console=ttyS0,115200 nokaslr debug'
